...
### 동기(Synchronous)
- 작업이 순차적으로 실행되며, 한 작업이 완료될 때까지 다음 작업이 대기합니다(블로킹).
- I/O 작업 중 CPU가 대기 상태가 되어 자원이 낭비되고 여러 요청 처리 시 병목이 발생합니다.

### 비동기(Asynchronous)
- 작업 완료를 기다리지 않고 다음 작업을 수행하는 논블로킹 방식입니다.
- Python에서는 `async`/`await` 구문으로 구현하며, I/O 작업 중에도 CPU를 효율적으로 활용합니다.

### 동시성(Concurrency)과 병렬성(Parallelism)
- **동시성**: 단일 코어에서 여러 작업을 번갈아 처리하는 논리적 동시 실행으로, I/O 바운드 작업에 효과적합니다. Python에서는 `async`/`await` 구문으로 구현하며, 코루틴이 I/O를 기다리는 동안 이벤트 루프가 다른 태스크로 전환합니다.
- **병렬성**: 여러 CPU 코어가 작업을 물리적으로 동시에 처리하는 방식으로, CPU 바운드 작업에 적합합니다.

### 멀티태스킹의 두 가지 방식
- **선점형 멀티태스킹**: OS가 작업에 시간을 할당하고 강제로 전환합니다. 멀티스레딩과 멀티프로세싱이 이 방식을 사용합니다.
- **협력적 멀티태스킹**: 작업이 스스로 제어권을 양보할 때만 전환됩니다. Python의 `async/await`이 이 방식으로 동작하며 I/O 작업에서 효율적입니다.
